...
;; 执行一个表达式
;; 表达式的执行有两种结果 1 求值结果 2 附加结果(format 打印输出)
;; 第一种方式 会将求值结果和附加结果都,放入到*scratch*,
;; 另外两种会将求值结果输出到小缓冲区,而将附加结果放入就到 sbcl中
;; C-j       (最后一个括号之后, 这个方式可能只能用于elisp??)
;; C-M-x     (表达式内部)
;; C-x C-e   (最后一个括号之后)(正确执行)
